...
#else
	if (!skb_csum_unnecessary(skb) && tcp_v6_checksum_init(skb)) {
           const struct ipv6hdr *ip_hdr = ipv6_hdr(skb);
           const struct tcphdr *th = tcp_hdr(skb);
	    printk(KERN_INFO "Bad Checksum, Next Header : %d, TCP seq #: %d\n", ip_hdr->nexthdr, ntohl(th->seq));
	}
       else {
           const struct ipv6hdr *ip_hdr = ipv6_hdr(skb);
           const struct tcphdr *th = tcp_hdr(skb);
	    printk(KERN_INFO "Good Checksum, Next Header : %d, TCP seq #: %d\n", ip_hdr->nexthdr, ntohl(th->seq));
       }
#endif
